...
@pytest.mark.anyio
async def test_connection_pool_reuses_new_connection_within_assignment_pass():
    pool = httpcore.AsyncConnectionPool(max_connections=10, http2=True)
    pool._requests = [
        AsyncPoolRequest(httpcore.Request("GET", "https://example.com/"))
        for _ in range(10)
    ]
...
@pytest.mark.anyio
async def test_connection_pool_does_not_multiplex_new_http11_connections():
    pool = httpcore.AsyncConnectionPool(max_connections=10)
    pool._requests = [
        AsyncPoolRequest(httpcore.Request("GET", "https://example.com/"))
        for _ in range(10)
    ]
